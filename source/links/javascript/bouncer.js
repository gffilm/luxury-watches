	...
	*/
	getUrlParams = function(param, searcher) {
		if (!searcher) {
			...
			(new RegExp('[?|&]' + param + '=' + '([^&;]+?)(&|#|;|$)').exec(searcher) ||
  			[,""])[1].replace(/\+/g, '%20')) || null;
	};
	...
	*/
	popWindow = function(relative) {
		...
		try {
			...
			setMessage('It appears a pop-up blocker is enabled, please disable pop up blockers for this site and refresh the page.');
